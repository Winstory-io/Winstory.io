    ...
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ color: '#fff', marginBottom: 18, fontWeight: 600 }}>
          ...
          <span style={{ color: '#FFD600', fontWeight: 700 }}>[X items]</span> = [Amount per user] × 3
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <input type="number" placeholder="How many item(s) per 3 best completers ?" style={{ padding: 12, borderRadius: 8, border: '2px solid #FFD600', background: 'none', color: '#FFD600', fontWeight: 600, fontSize: 16 }} />
        </div>
      </div>
    </div>
  );
