...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'winc_reward_type') THEN
...
-- Types de staking
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'stake_type') THEN
...
-- Types de métriques
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'metric_period') THEN
...
-- Types de modération
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'moderation_level') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'blockchain_validation_type') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'minting_status') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'ip_reputation_score') THEN
...
-- Auto resolve policy pour la modération hybride
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS creator_infos (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE TABLE IF NOT EXISTS campaign_pricing_configs (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT UNIQUE NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS campaign_rewards_configs (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE TABLE IF NOT EXISTS digital_access_rewards (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  reward_tier reward_tier NOT NULL,
...
CREATE TABLE IF NOT EXISTS physical_access_rewards (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  reward_tier reward_tier NOT NULL,
  access_name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS mint_payments (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS winc_rewards (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE TABLE IF NOT EXISTS user_xp_progression (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  user_wallet TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS digital_access_deliveries (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE TABLE IF NOT EXISTS physical_access_deliveries (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  completion_id TEXT NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS reward_fulfillment_jobs (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE TABLE IF NOT EXISTS completion_nft_assignments (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  completion_id TEXT NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS reward_eligibility_rules (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  reward_tier reward_tier NOT NULL,
...
CREATE TABLE IF NOT EXISTS moderation_progress (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS moderation_eligibility (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  moderator_wallet TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS moderator_stakes (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  moderator_wallet TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS staking_configuration (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS moderation_vote_justifications (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE TABLE IF NOT EXISTS completion_validation_conditions (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  completion_id TEXT NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS campaign_completion_links (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  original_campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS completion_deadlines (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS completion_content_metadata (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  completion_id TEXT NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS completion_moderation_scores (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  completion_id TEXT NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
  moderator_wallet TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS suspicious_behavior_detection (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS winstory_interventions (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS automatic_validation_tracking (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  completion_id TEXT REFERENCES completions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS automatic_distribution_logs (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  completion_id TEXT REFERENCES completions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS campaign_reward_networks (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  reward_type reward_type NOT NULL,
...
CREATE TABLE IF NOT EXISTS blockchain_transaction_queue (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS blockchain_transaction_receipts (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE OR REPLACE VIEW completions_with_validation AS
SELECT 
  co.*,
  c.title as campaign_title,
  c.creator_type,
...
CREATE TABLE IF NOT EXISTS user_completion_history (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  user_wallet TEXT NOT NULL,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  completion_id TEXT NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS user_completion_moderator_scores (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
CREATE TABLE IF NOT EXISTS user_earned_rewards (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  user_wallet TEXT NOT NULL,
  campaign_id TEXT NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  completion_id TEXT REFERENCES completions(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS notification_message_logs (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
...
-- =====================================================
-- TABLES SUPER-MODÉRATEUR
-- =====================================================

-- Table pour les votes Super-Modérateur
CREATE TABLE IF NOT EXISTS super_moderator_votes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id UUID NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  completion_id UUID NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
  super_moderator_wallet TEXT NOT NULL,
  vote_decision moderation_decision NOT NULL,
  score INTEGER CHECK (score >= 1 AND score <= 100),
  final_score DECIMAL(5,2),
  final_decision moderation_decision,
  community_score DECIMAL(5,2),
  community_decision moderation_decision,
...
-- Table pour les autorisations Super-Modérateur
CREATE TABLE IF NOT EXISTS super_moderator_permissions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id UUID NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  super_moderator_wallet TEXT NOT NULL,
  ...
  revoked_at TIMESTAMP WITH TIME ZONE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(campaign_id, super_moderator_wallet)
);

-- Table pour l'historique des décisions Super-Modérateur
CREATE TABLE IF NOT EXISTS super_moderator_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id UUID NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
  completion_id UUID NOT NULL REFERENCES completions(id) ON DELETE CASCADE,
  super_moderator_wallet TEXT NOT NULL,
  action_type TEXT NOT NULL CHECK (action_type IN ('vote_cast', 'decision_override', 'score_adjustment')),
  previous_decision moderation_decision,
  new_decision moderation_decision,
...
-- =====================================================
-- INDEX SUPER-MODÉRATEUR
-- =====================================================

-- Index pour les votes Super-Modérateur
CREATE INDEX IF NOT EXISTS idx_super_moderator_votes_campaign ON super_moderator_votes(campaign_id);
CREATE INDEX IF NOT EXISTS idx_super_moderator_votes_completion ON super_moderator_votes(completion_id);
CREATE INDEX IF NOT EXISTS idx_super_moderator_votes_wallet ON super_moderator_votes(super_moderator_wallet);
CREATE INDEX IF NOT EXISTS idx_super_moderator_votes_decision ON super_moderator_votes(vote_decision);
CREATE INDEX IF NOT EXISTS idx_super_moderator_votes_created ON super_moderator_votes(created_at);

-- Index pour les autorisations Super-Modérateur
CREATE INDEX IF NOT EXISTS idx_super_moderator_permissions_campaign ON super_moderator_permissions(campaign_id);
CREATE INDEX IF NOT EXISTS idx_super_moderator_permissions_wallet ON super_moderator_permissions(super_moderator_wallet);
CREATE INDEX IF NOT EXISTS idx_super_moderator_permissions_authorized ON super_moderator_permissions(is_authorized);

-- Index pour l'historique Super-Modérateur
CREATE INDEX IF NOT EXISTS idx_super_moderator_history_campaign ON super_moderator_history(campaign_id);
CREATE INDEX IF NOT EXISTS idx_super_moderator_history_completion ON super_moderator_history(completion_id);
CREATE INDEX IF NOT EXISTS idx_super_moderator_history_wallet ON super_moderator_history(super_moderator_wallet);
CREATE INDEX IF NOT EXISTS idx_super_moderator_history_action ON super_moderator_history(action_type);
CREATE INDEX IF NOT EXISTS idx_super_moderator_history_created ON super_moderator_history(created_at);

-- =====================================================
-- FONCTIONS SUPER-MODÉRATEUR
-- =====================================================

-- Fonction pour vérifier les autorisations Super-Modérateur
CREATE OR REPLACE FUNCTION check_super_moderator_authorization(
  p_campaign_id UUID,
  p_wallet TEXT
) RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
  ...
  SELECT creator_wallet INTO campaign_creator
  FROM campaigns
  WHERE id = p_campaign_id;
  
  IF campaign_creator = p_wallet THEN
    is_authorized := TRUE;
  END IF;
  
  -- Vérifier les autorisations explicites
  SELECT COALESCE(is_authorized, FALSE) INTO is_authorized
  FROM super_moderator_permissions
  WHERE campaign_id = p_campaign_id 
    AND super_moderator_wallet = p_wallet
...
CREATE OR REPLACE FUNCTION calculate_super_moderator_final_score(
  p_community_score DECIMAL,
  p_super_moderator_score DECIMAL
) RETURNS DECIMAL AS $$
BEGIN
  -- Formule: (Communauté × 49%) + (Super-Modérateur × 51%)
  RETURN ROUND((p_community_score * 0.49) + (p_super_moderator_score * 0.51), 2);
END;
$$ LANGUAGE plpgsql;

-- Fonction pour déterminer la décision finale
CREATE OR REPLACE FUNCTION calculate_super_moderator_final_decision(
  p_community_decision moderation_decision,
  p_super_moderator_decision moderation_decision
) RETURNS moderation_decision AS $$
BEGIN
  -- Le Super-Modérateur peut basculer la décision communautaire
  RETURN p_super_moderator_decision;
END;
$$ LANGUAGE plpgsql;

-- =====================================================
-- TRIGGERS SUPER-MODÉRATEUR
-- =====================================================

-- Trigger pour mettre à jour updated_at sur super_moderator_votes
CREATE OR REPLACE FUNCTION update_super_moderator_votes_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_super_moderator_votes_updated ON super_moderator_votes;
CREATE TRIGGER trg_super_moderator_votes_updated
BEFORE UPDATE ON super_moderator_votes
FOR EACH ROW EXECUTE FUNCTION update_super_moderator_votes_updated_at();

-- Trigger pour enregistrer l'historique des votes Super-Modérateur
CREATE OR REPLACE FUNCTION log_super_moderator_vote()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO super_moderator_history (
    campaign_id,
    completion_id,
    super_moderator_wallet,
    action_type,
    new_decision,
    new_score,
    metadata
    ...
    jsonb_build_object(
      ...
      'community_decision', NEW.community_decision,
      'calculation_breakdown', NEW.calculation_breakdown
    )
  );
...
DROP TRIGGER IF EXISTS trg_super_moderator_vote_logged ON super_moderator_votes;
CREATE TRIGGER trg_super_moderator_vote_logged
AFTER INSERT ON super_moderator_votes
FOR EACH ROW EXECUTE FUNCTION log_super_moderator_vote();

-- =====================================================
-- VUES SUPER-MODÉRATEUR
-- =====================================================

-- Vue pour les statistiques Super-Modérateur
CREATE OR REPLACE VIEW super_moderator_stats AS
SELECT 
  smv.campaign_id,
  smv.completion_id,
  smv.super_moderator_wallet,
  COUNT(*) as total_votes,
  COUNT(CASE WHEN smv.vote_decision = 'VALID' THEN 1 END) as valid_votes,
  COUNT(CASE WHEN smv.vote_decision = 'REFUSE' THEN 1 END) as refuse_votes,
  AVG(smv.score) as average_score,
  AVG(smv.final_score) as average_final_score,
  COUNT(CASE WHEN smv.vote_decision != smv.community_decision THEN 1 END) as override_count,
  MIN(smv.created_at) as first_vote_at,
  MAX(smv.created_at) as last_vote_at
FROM super_moderator_votes smv
GROUP BY smv.campaign_id, smv.completion_id, smv.super_moderator_wallet;

-- Vue pour les complétions en attente de Super-Modération
CREATE OR REPLACE VIEW pending_super_moderation AS
SELECT 
  c.id as completion_id,
  c.campaign_id,
  c.title as completion_title,
  c.completer_wallet,
  c.created_at as completion_date,
  mp.valid_votes,
  mp.refuse_votes,
  mp.average_score,
  mp.decision as community_decision,
  mp.stakers_count,
  mp.staking_pool_total,
  CASE 
    WHEN smv.id IS NOT NULL THEN 'moderated'
    WHEN mp.decision IS NOT NULL AND mp.valid_votes + mp.refuse_votes >= 22 THEN 'pending'
    ELSE 'waiting_community'
  END as moderation_status
FROM completions c
LEFT JOIN moderation_progress mp ON c.id = mp.completion_id
LEFT JOIN super_moderator_votes smv ON c.id = smv.completion_id
WHERE c.status = 'PENDING_MODERATION'
  AND mp.decision IS NOT NULL
  AND mp.valid_votes + mp.refuse_votes >= 22;

-- =====================================================
-- COMMENTAIRES SUPER-MODÉRATEUR
-- =====================================================

COMMENT ON TABLE super_moderator_votes IS 'Votes des Super-Modérateurs avec pondération 51/49';
COMMENT ON TABLE super_moderator_permissions IS 'Autorisations des Super-Modérateurs par campagne';
COMMENT ON TABLE super_moderator_history IS 'Historique des actions Super-Modérateur';

COMMENT ON COLUMN super_moderator_votes.score IS 'Score Super-Modérateur (1-100)';
COMMENT ON COLUMN super_moderator_votes.final_score IS 'Score final calculé avec pondération 51/49';
COMMENT ON COLUMN super_moderator_votes.final_decision IS 'Décision finale (peut basculer la décision communautaire)';
COMMENT ON COLUMN super_moderator_votes.calculation_breakdown IS 'Détails du calcul JSON';

COMMENT ON FUNCTION check_super_moderator_authorization IS 'Vérifie si un wallet peut exercer le rôle Super-Modérateur';
COMMENT ON FUNCTION calculate_super_moderator_final_score IS 'Calcule le score final avec pondération 51/49';
COMMENT ON FUNCTION calculate_super_moderator_final_decision IS 'Détermine la décision finale (pouvoir de basculement)';